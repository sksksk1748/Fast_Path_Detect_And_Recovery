...
"""
def get_linkdown():
    response = requests.get("http://127.0.0.1:8080/switch/link_down")
    return response.json()
"""
2020/12/05 test http link by tim
"""
...
class Shell(cmd.Cmd):
    ...
    def do_linkdown(self,line):
        """Command: link\nDescription: Show the link"""
        Shell.link=get_linkdown()
        #print(Shell.link)
        Shell.host=get_host()
        for switch1 in Shell.link:
            for x in Shell.link[switch1]:
            #for port1 in switch1:
                #for h in Shell.host:
                print("Switch["+str(x)+"] ----- port["+str(switch1)+"]")
                    #print "(s"+switch1+" ----- "+h+")"
